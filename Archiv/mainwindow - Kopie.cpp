...
void MainWindow::process(){
    ...
    connect(ui->fieldE10,SIGNAL(clicked(bool)),ui->FieldE8to9,SLOT(scrollUp()));
    connect(ui->fieldE11,SIGNAL(clicked(bool)),ui->FieldE8to9,SLOT(scrollDown()));
    connect(ui->FieldE8to9,SIGNAL(enableScrollUp(bool)),ui->fieldE10,SLOT(setEnabled(bool)));
    connect(ui->FieldE8to9,SIGNAL(enableScrollDown(bool)),ui->fieldE11,SLOT(setEnabled(bool)));
    ...
    connect(mySep,SIGNAL(newLzbIndicators(QVector<quint8>)),myLzb,SLOT(setIndicators(QVector<quint8>)));
  //connect(myTcp->myIndicators,SIGNAL(newLzbIndicators(QVector<quint8>)),myLzb,SLOT(setIndicators(QVector<quint8>)));
    connect(myTcp,SIGNAL(newLzbIndicators(QVector<quint8>)),myLzb,SLOT(setIndicators(QVector<quint8>)));
    connect(myLzb,SIGNAL(newLzbTextMessage(QString, QColor, QColor, quint8)),ui->FieldELzbTexts,SLOT(addTextLzbMessage(QString, QColor, QColor, quint8)));
    //connect(myTcp->myIndicators,SIGNAL(newTechnicalMessage(QStringList, QStringList)),this,SLOT(setEtcsTexs(QStringList, QStringList))); // FIXME: Need replace
  ...
  //connect(myTcp->myIndicators,SIGNAL(newLzbValues(QVector<quint8>)),myLzb,SLOT(setAnalogValues(QVector<quint8>)));
    ...
    connect(myLzb,SIGNAL(speedGt0(bool)),this,SLOT(enableTde(bool)));
    connect(myLzb,SIGNAL(speedGt40(bool)),this,SLOT(anableOverride(bool)));
    ...
    connect(myTcp,SIGNAL(changedTrain()), this,SLOT(initialize()));
    connect(myTcp,SIGNAL(changedTrain()), ui->widgetTacho,    SLOT(reset()));
    ...
    connect(myTcp->myEtcs,SIGNAL(newTextMessages(QList<EtcsTextMessage>)),ui->FieldE8to9,SLOT(setEtcsTextMessages(QList<EtcsTextMessage>)));
    connect(myTcp        ,SIGNAL(newTextMessages(QList<EtcsTextMessage>)),ui->FieldE8to9,SLOT(setEtcsTextMessages(QList<EtcsTextMessage>)));
    /*connect(myTcp->myEtcs, &zusi3etcs::newTextMessages,
            ui->FieldE8to9, &EtcsTextWidget::setEtcsTextMessages);*/
    connect(ui->FieldE8to9, SIGNAL(sendAckEtcsTextMessage(quint32)), myTcp, SLOT(sendAckEtcsTextMessage(quint32)));
    ...
    connect(myLzb,SIGNAL(lzbAvailable(bool)),this,SLOT(setLzbPresent(bool)));
    ...
    connect(myZusiTrainData,SIGNAL(newTraindataEtcs(quint16, QString, quint16, quint16, QString, QString, QString, bool)),myTcp,SLOT(setTraindataEtcs(quint16, QString, quint16, quint16, QString, QString, QString, bool)));
    ...
    connect(mySep,SIGNAL(tcpConnectionSettings(quint8)),myTcp,SLOT(setAutoReconnect(quint8)));
...
void MainWindow::connectTcpStuff(){
    ...
    connect(myLzb,SIGNAL(newLevelInforamtion(quint16)),this,SLOT(setLevel(quint16)));
}
